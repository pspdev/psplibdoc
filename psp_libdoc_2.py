...
NIDEntry = namedtuple('NIDEntry', ['nid', 'name', 'prx', 'prxName', 'libraryName', 'libraryFlags', 'firmwareVersion'])

def compute_nid(name):
	return hashlib.sha1(name.encode('ascii')).digest()[:4][::-1].hex().upper()

def loadPSPLibdoc(xmlFile, firmVersion = ''):
...
def exportPSPLibdocCombined(nidEntries, outFile):
	...
	for entry in entries:
		...
		if lastLibrary != entry.libraryName:
			if lastLibrary != "":
				for (nid, curNid) in curNids.items():
					name = curNid["name"]
					versions = curNid["versions"]
					status = curNid["status"]
					function = ET.SubElement(functions, "FUNCTION")

					nid_elem = ET.SubElement(function, "NID")
					nid_elem.text = "0x" + nid

					name_elem = ET.SubElement(function, "NAME")
					name_elem.text = name
					...
					if status != 'unknown':
						...
						elif name in confirmedNames:
							source.text = "previous version"
						else:
							source.text = "unknown"

					versions_elem = ET.SubElement(function, "VERSIONS")
					for ver in versions:
		...
		if entry.name.endswith(entry.nid):
			status = 'unknown'
		elif compute_nid(entry.name) == entry.nid:
			status = 'matching'
			confirmedNames.append(entry.name)
		else:
			status = 'wrong'
		if entry.nid in curNids:
			if status == curNids[entry.nid]["status"] and entry.name == curNids[entry.nid]["name"]:
				pass # ok
			elif status == 'unknown' and curNids[entry.nid]["status"] in ['matching', 'wrong']:
				pass # former takes priority
			elif status in ['matching', 'wrong'] and curNids[entry.nid]["status"] == 'unknown':
				# current takes priority
				curNids[entry.nid]["status"] = status
				curNids[entry.nid]["name"] = entry.name
			else:
				print("WARNING: cannot merge names {} and {} for NID {}".format(curNids[entry.nid]["name"], entry.name, entry.nid))
			curNids[entry.nid]["versions"].append(entry.firmwareVersion)
		else:
			curNids[entry.nid] = {"status": status, "name": entry.name, "versions": [entry.firmwareVersion]}
...
if __name__ == '__main__':
	...
	if(args.libdoc):
		for libdoc in args.libdoc:
			splittedPath = libdoc.split('/')
			if splittedPath[0] == 'ePSPVitaLibDoc':
				firmVer = 'vita-' + splittedPath[1]
			else:
				firmVer = splittedPath[1]
			libdocEntries = loadPSPLibdoc(libdoc, firmVer)
